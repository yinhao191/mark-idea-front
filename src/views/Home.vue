    ...
    <el-container style="height: 100vh">
      ...
      >
        <div>
          ...
          <span
            style="color: rgb(248, 248, 248); font-size: 25px; margin-top: 20px;"
            class="fa fa-bars"
          ></span>
          ...
          <el-dropdown class="pull-right" placement="top-start">
            <span class="el-dropdown-link">
              <i
                style="color: black; font-size: 25px; margin-top: 20px; "
                class="fa fa-bars noselect"
              ></i>
            </span>
            <el-dropdown-menu slot="dropdown">
              <router-link style="color: black ; text-decoration: none" to="/setting">
                <el-dropdown-item class="markidea-dropdown-item">设置</el-dropdown-item>
              </router-link>
              <router-link style="color: black ; text-decoration: none" to="/admin">
                <el-dropdown-item v-if="isAdminUser" class="markidea-dropdown-item">网站管理</el-dropdown-item>
              </router-link>
              <el-dropdown-item
                class="markidea-dropdown-item"
                divided
      ...
      <el-container>
        ...
        <el-aside width="200px" class="notebooklist noselect" v-show="showAside ">
            ...
            <el-popover v-model="newNoteBookVisible" placement="bottom">
              <div>
                <el-input
                  ...
                  @keyup.enter.native="handleCreateNotebook"
                  placeholder="新笔记本名"
                />
              </div>
          ...
          >
            ...
            >
              <el-input
                @blur="toRenameNotebookName = null"
                v-model="destNotebookName"
                @keyup.enter.native="handleRenameNotebook"
                placeholder="新笔记本名"
              />
            </div>
          ...
          >
            <el-collapse-item style="background-color: rgb(248, 248, 248);">
              <template slot="title">
                <div
                  v-contextmenu:delNotesRightMenu
                  class="notebook"
...
"
              ...
              <div
                v-contextmenu:delNoteRightMenu
                class="delnote"
                v-for="item of delNoteList"
                :key="item.id"
              >{{item.notebook}}/{{item.title}}</div>
            </el-collapse-item>
          </el-collapse>
        </el-aside>
              ...
              >
                <div>
                  <el-input
                    ...
                    @keyup.enter.native="handleCreateNote"
                    placeholder="新笔记名"
                  />
                </div>
                <i
  ...
  <i v-if = "item.status === 2" aria-label="修改尚未保存" style = "color:lightgrey" 
                ...
                <span class="vditor-tooltipped vditor-tooltipped__nw" aria-label="公开笔记">
                  <i
                    v-if="item.articleId"
                    style="color:lightgrey;padding-left:5px"
                    class="fa fa-eye"
                  ></i>
                </span>
              </span>
        ...
        <el-main v-show="!isMobile || !showAside" class="editor" style="overflow: unset">
          <Editor
            ...
            @renameTitle="doHandleRenameTitle"
          />
        </el-main>
          ...
          <div style="padding-left: 10px;padding-top: 15px;">
            ...
            <div v-for="version of curNoteVersion" :key="version.ref">
              ...
              <el-button
                ...
                type="text"
                style="font-size:13px;"
                @click="handlePreviewHistory(version.ref)"
              >预览</el-button>
            </div>
          </div>
        </el-aside>
    ...
    >
      ...
      <v-contextmenu-submenu title="移动至">
        <div v-for="otherNotebook of notebookList" :key="otherNotebook.notebookName">
          <v-contextmenu-item
            @click="handleMoveNote(otherNotebook.notebookName)"
            v-if="curNotebook.notebookName !== otherNotebook.notebookName"
          >📙 {{otherNotebook.notebookName}}</v-contextmenu-item>
        </div>
      </v-contextmenu-submenu>
      <v-contextmenu-submenu title="复制到">
        <div v-for="otherNotebook of notebookList" :key="otherNotebook.notebookName">
          <v-contextmenu-item
            @click="handleCopyNote(otherNotebook.notebookName)"
            v-if="curNotebook.notebookName !== otherNotebook.notebookName"
          >📙 {{otherNotebook.notebookName}}</v-contextmenu-item>
        </div>
      </v-contextmenu-submenu>
    ...
    <v-contextmenu
      theme="dark"
      class="rightMenu"
      ref="notebookRightMenu"
      @contextmenu="handleNotebookRightMenu"
    >
    ...
    <v-contextmenu
      theme="dark"
      class="rightMenu"
      @contextmenu="handleDelNoteRightMenu"
      ref="delNoteRightMenu"
    >
  ...
  methods: {
    ...
    selectNoteList(notebookName) {
      ...
      if (this.isModifUnsaved()) {
        this.$confirm("修改尚未保存", "Confirm", {
          ...
          .then(() => {
            this.handleSaveContentAndSwitchNotebook(
          ...
          .catch(action => {
            this.$notify({
              ...
              message: action === "cancel" ? "丢弃修改" : "停留在当前页",
              duration: 1500
            });
            if (action === "cancel") {
    ...
    selectNote(noteTitle, notebookName) {
      ...
      if (
        ...
        notebookName === this.curNote.notebookName
      ) {
        return;
      }
    ...
    handleSaveContentAndSwitchNotebook(content, notebookName) {
      ...
      let url =
        global.HOST_URL +
        "/note/" +
        this.curNote.notebookName +
        "/" +
        this.curNote.noteTitle;
    ...
    handleDelNotebook() {
      let url =
        global.HOST_URL + "/note/" + this.notebookRightMenuValues.notebookName;
      axios.delete(url, this.config).then(res => {
        res = res.data;
        if (res.code === 0) {
    ...
    handleDelNote() {
      let url =
        global.HOST_URL +
        "/note/" +
        this.noteRightMenuValues.notebookName +
        "/" +
        this.noteRightMenuValues.noteTitle;
      axios.delete(url, this.config).then(res => {
        res = res.data;
        if (res.code === 0) {
    ...
    isModifUnsaved(raw) {
      if (
        this.$refs.editor.getContent(raw).charCodeAt() === 10 &&
        this.curNote.content === ""
      ) {
        return false;
      }
    ...
    autoSaveNote() {
      ...
      if (
        !this.curNote ||
        !this.curNote.noteTitle ||
        this.curNote.noteTitle.length === 0
      ) {
        return;
      }
